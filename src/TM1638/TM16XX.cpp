               ...
               byte intensity)
{
  ...
  sendCommand(0x40);
  sendCommand(0x80 | (activateDisplay ? 8 : 0) | min(7, (int)intensity));
...
{
  sendCommand(0x80 | (active ? 8 : 0) | min(7, (int)intensity));
...
void TM16XX::setDisplayToString(const char *string, const bool shouldBlink, const word dots, const byte pos, const byte font[])
{
...
void TM16XX::setDisplayToString(const String string, const bool shouldBlink, const word dots, const byte pos, const byte font[])
{
  static unsigned long lastUpdateTime = 0;
  static bool displayOn = true;
  const unsigned long blinkInterval = 500;
